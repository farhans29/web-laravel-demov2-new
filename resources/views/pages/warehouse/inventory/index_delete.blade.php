<x-app-layout>
    ...
    <script>
        ...
        function updateInventoryTable() {
            fetch('/warehouse/inventory/delete') // Ganti dengan URL yang sesuai untuk mengambil data inventaris
                .then(response => response.json())
                .then(data => {
                    ...
                    data.inventories.forEach(inventory => {
                        ...
                        row.className = 'hover:bg-gray-100 odd:bg-gray-100 even:bg-white';
                        row.setAttribute('data-id', inventory.id_inventory);
                        row.innerHTML = `
                                            <td class="px-6 py-4 whitespace-nowrap">${inventory.id_inventory}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">${inventory.category}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">${inventory.name}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">${number_format(inventory.qty, 0, '', '.')}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">${inventory.unit}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">${number_format(inventory.net_weight, 0, '', '.')} ${inventory.w_unit}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-center">${number_format(inventory.price_list, 0, '', '.')}</td>
                                            <td class="px-6 py-4 whitespace-nowrap text-left flex gap-x-2">
                                                <button onclick="confirmDelete('${inventory.id_inventory}')" class="px-4 py-2 bg-red-600 text-white rounded-md hover:bg-red-700">Delete</button>
                                            </td>
                                        `;
